...
save(biological_age0, file = "/home/esaha/aging_plots/TCGA_biological_age.RData")

# Plot age vs biological age
library(ggplot2)
fulldata = data.frame(age)
fulldata$aging_score = biological_age
...
ggplot(fulldata, aes(x = age , y = aging_score, col = tumor_stage)) +
  geom_point(size=1.5) + labs(title="Network-informed Aging score in Tumor (TCGA)",
                                                              y="aging score", x = "age")
ggplot(fulldata, aes(x = tumor_stage , y = aging_score, col = tumor_stage)) +
  geom_boxplot() + labs(title="Boxplot of Aging score by Tumor Stage",
                            y="aging score", x = "tumor stage") + scale_y_continuous(limits = quantile(fulldata$aging_score, c(0.1, 0.9)))
#geom_smooth(method = "lm", fill = NA) + 
pval_gender = wilcox.test(fulldata$aging_score[which(fulldata$gender == "MALE")], fulldata$aging_score[which(fulldata$gender == "FEMALE")])
pval_gender = round(pval_gender$p.value,3)
ggplot(fulldata, aes(x = gender , y = aging_score, col = gender)) +
  geom_boxplot() + labs(title=paste("Boxplot of Aging score by Gender (p-val = ",pval_gender, ")", sep = ""),
                        y="aging score", x = "gender") 

pval_race = anova(lm(fulldata$aging_score~fulldata$race))
pval_race = round(pval_race$`Pr(>F)`[1],3)
ggplot(fulldata, aes(x = race , y = aging_score, col = race)) +
  geom_boxplot() + labs(title=paste("Boxplot of Aging score by Race (p-val = ",pval_race, ")", sep = ""),
                        y="aging score", x = "gender") 
...
ggsurv(fit, CI=FALSE,  plot.cens = F, cens.col = "gray", surv.col=c("red", "orange"),
       xlab = "Time (years)", ylab = "Overall survival probability",
       main="Survival probability by Aging Signature (TCGA Female)") +
  coord_cartesian(ylim=c(0,1)) +
  theme(text=element_text(size=12), axis.text=element_text(size=10), plot.title=element_text(size=16),
        legend.title=element_blank(), legend.justification=c(1,0), legend.position=c(0.9,0.8)) +
  geom_text(x=1.2, y=0.05, label=paste0("p=",pval), size=5, family="Helvetica")

SurvObj0 = SurvObj[which(race == "white"),]
biological_age_group0 = biological_age_group[which(race == "white")]
fit = survfit(SurvObj0 ~ biological_age_group0)
diff <- survdiff(SurvObj0 ~ biological_age_group0)
pval <- round(1 - pchisq(diff$chisq, length(diff$n) - 1),3)
ggsurv(fit, CI=FALSE,  plot.cens = F, cens.col = "gray", surv.col=c("red", "orange"),
       xlab = "Time (years)", ylab = "Overall survival probability",
       main="Survival probability by Aging Signature (TCGA White)") +
  coord_cartesian(ylim=c(0,1)) +
  theme(text=element_text(size=12), axis.text=element_text(size=10), plot.title=element_text(size=16),
        legend.title=element_blank(), legend.justification=c(1,0), legend.position=c(0.9,0.8)) +
  geom_text(x=1.2, y=0.05, label=paste0("p=",pval), size=5, family="Helvetica")
...
ggsurv(fit, CI=FALSE,  plot.cens = F, cens.col = "gray", surv.col=c("red", "orange"),
       xlab = "Time (years)", ylab = "Overall survival probability",
       main="Survival probability by Chronological Age (TCGA)") +
  coord_cartesian(ylim=c(0,1)) +
  theme(text=element_text(size=12), axis.text=element_text(size=10), plot.title=element_text(size=16),
        legend.title=element_blank(), legend.justification=c(1,0), legend.position=c(0.9,0.8)) +
  geom_text(x=1.2, y=0.05, label=paste0("p=",pval), size=5, family="Helvetica")
...
# cox_tcga <- coxph(SurvObj ~ biological_age*therapy + tumor_stage + gender + smoking_status + race)
cox_tcga <- coxph(SurvObj ~ biological_age*therapy)
cox_results = summary(cox_tcga)$coefficients

# cox_tcga <- coxph(SurvObj ~ age*therapy + tumor_stage + gender + smoking_status + race)
cox_tcga <- coxph(SurvObj ~ age*therapy)
cox_results = summary(cox_tcga)$coefficients

############ Table for Pathway Score Coefficients ########################
