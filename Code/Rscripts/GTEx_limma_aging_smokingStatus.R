...
#### Design Matrix
design = model.matrix(~ age*smoking_status + gender + race + smoking_status + ischemic_timeH )
...
write.table(tb, file = "/home/esaha/Recount3_GTEx_TCGA_Data/TPM_expression/newdata2023_aging/limma_GSEA_aging/limma_GTEx_nonsmoker_aging.txt", col.names = T, row.names = T)
save(fgseaRes, file = "/home/esaha/Recount3_GTEx_TCGA_Data/TPM_expression/newdata2023_aging/limma_GSEA_aging/gsea_GTEx_nonsmoker_aging.RData")

#######################################
colnames(design) = make.names(colnames(design))
contrast.matrix <- makeContrasts(age_smoker = age + age.smoking_statusYes, levels=design)
colnames(fit$coefficients) = rownames(contrast.matrix)
fit2 <- contrasts.fit(fit, contrast.matrix)
fit2 <- eBayes(fit2)
tb = topTable(fit2,coef="age_smoker",number=Inf)
tb$chr = gene_info.TCGA$seqnames[match(rownames(tb), gene_info.TCGA$gene_id)]
tb$gene_name = gene_info.TCGA$gene_name[match(rownames(tb), gene_info.TCGA$gene_id)]
head(tb)
...
write.table(tb, file = "/home/esaha/Recount3_GTEx_TCGA_Data/TPM_expression/newdata2023_aging/limma_GSEA_aging/limma_GTEx_smoker_aging.txt", col.names = T, row.names = T)
save(fgseaRes, file = "/home/esaha/Recount3_GTEx_TCGA_Data/TPM_expression/newdata2023_aging/limma_GSEA_aging/gsea_GTEx_smoker_aging.RData")
